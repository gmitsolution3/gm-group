...
interface AccountAnalysisDashboardErrorProps {
  message?: string;
  onRetry: () => void;
}

export function AccountAnalysisDashboardError({
  message,
  onRetry,
}: AccountAnalysisDashboardErrorProps) {
  return (
      ...
      <div className="relative mx-auto flex min-h-[520px] w-full max-w-[1440px] items-center justify-center overflow-hidden rounded-3xl border border-red-100 bg-gradient-to-br from-red-50/70 via-background to-background p-8 sm:p-12">
        ...
        <div className="relative w-full max-w-lg text-center">
          ...
          <h1 className="mt-5 font-display text-2xl font-bold tracking-tight sm:text-3xl">
            We couldn't load the account analysis
          </h1>

          {/* Description */}
          <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground sm:text-base">
            {message ||
              "We couldn't retrieve the latest financial and operational statistics. This may be a temporary connection problem."}
          </p>
          ...
          <p className="mt-5 text-xs text-muted-foreground/70">
            Your account data has not been changed. Please try again
            in a few moments.
          </p>
        </div>
      </div>
